...
class StudentModelView(APIView):
    ...
    def get(self, request, id=-1):
        if id > -1:
            ...
            #stu = Student.objects.all()          # GETTING all data that all users posted
            stu = request.user.student_set.all()  # GETTING data per user - django4kids: i want to GET only the data that the logged in POSTED          
            stu_serialized = StudentSerializer(stu, many=True) 
            return Response(stu_serialized.data)
        
    def post(self, request):
        # print(f'{request.user} is the user !!!!!!') # add User to Students table : lets see who is the user in the system
        stu = StudentSerializer(data=request.data, context={'user': request.user}) # Getting data per user - django4kids: using context we pass request.user to the serializer so the object instansiated populate the new field user.
        if stu.is_valid():
            stu.save()
